...
if (!existsSync(outputDir)) {
  throw new Error(`missing build output at ${relative(rootDir, outputDir)} — run "vite build" first`);
}
...
function optimizeGroupAvatarThumbnails() {
  ...
  content = content.replace(
    ...
    function (match, sourceRef, fileName) {
      ...
      const sourceFile = join(outputDir, sourceRef.slice(1));
...
console.log(`Optimized media in ${relative(rootDir, outputDir)}`);
